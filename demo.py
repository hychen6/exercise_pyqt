...
class MyWindow(QMainWindow, Ui_Form):
    def __init__(self, parent=None):
        ...
        data = np.sin(x) / x
        self.graphicsView.plot(x,data, pen=(255, 255, 255, 150))
        self.label = pg.TextItem(text="横坐标：{}".format(0))
        self.label1 = pg.TextItem(text='纵坐标:{}'.format(0))
        self.graphicsView.addItem(self.label,self.label1)
        self.graphicsView.setCursor()
    ...
    def onMouseMoved(self, evt):
        if self.graphicsView.plotItem.vb.mapSceneToView(evt):
            ...
            self.label1.setHtml("<p style='color:grey'>纵坐标：{0}</p>".format(point.y()))
